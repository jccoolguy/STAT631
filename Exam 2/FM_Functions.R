...
resid.summary<-function(res, labs = syb, plot = T){
	dimnames(res)[[2]] = labs
	N = dim(res)[2]
	...
	sum = cat(sum1, sum2)
	sum.plot = plot
	if(plot) sum.plot = cor.plot(cor(res))
...
##  RSq.all is a N by 3 matrix, use cbind() to combine R-Squares
##          from the 3 models
##	The columns are orderd as models
##__________________________________________________________
##

RSq.plot = function(RSq.all,  models = c("FF", "FA", "PC"), labs = syb, cols =c( "#EF3B2C" , "#3690C0", "#4DAF4A") ){
	RSq = RSq.all
	if(dim(RSq)[1] != length(models)) RSq = t(RSq)
	dimnames(RSq)[[2]] = labs; dimnames(RSq)[[1]] = models;
	par(mfrow = c(1,1), las = 1, pty = "m", mar = c(3.6,4.1,0.6,2.1), cex.axis = 0.8, mgp = c(2,0.75,0))
...
cor.test.all = function(res){
	cors = cor(res) ## correlation matrix
	rhos = cors[upper.tri(cors)] ## N*(N-1)/2 correlations
	t.stat = rhos*sqrt(n-2)/(1-rhos^2)
	sig.05 = sum(abs(t.stat) > qt(.975,n-2))
	sig.01 = sum(abs(t.stat) > qt(.995,n-2))
