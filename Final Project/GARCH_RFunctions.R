...
setMethod("rate0", 
    ...
    function(object){
        dist = object@model$modeldesc$distribution
        distpar = object@model$pars[c("sigma", "skew", "shape"),1];
        cri.u = qdist(dist, c(.95,.975), sigma = distpar["sigma"], skew = distpar["skew"], shape = distpar["shape"])
		cri.l = qdist(dist, c(.05,.025), sigma = distpar["sigma"], skew = distpar["skew"], shape = distpar["shape"])
        T0 = object@model$modeldata$T
        n.roll = object@forecast$n.roll
        ind.new = (T0+1):(T0+1+n.roll)
        y.new = object@model$modeldata$data[ind.new]
        y.hat = fitted(object)[1,]; 
        out.90 = c(mean(y.new < (y.hat + cri.l[1])), mean(y.new > (y.hat + cri.u[1])))
        out.90 = c(1-sum(out.90), out.90)
        out.95 = c(mean(y.new < (y.hat + cri.l[2])), mean(y.new > (y.hat + cri.u[2])))
        out.95 = c(1-sum(out.95), out.95)
        out = rbind(out.95, out.90)
        dimnames(out)[[1]] = c("95% PI", "90% PI")
        dimnames(out)[[2]] = c("coverage", "below PI", "beyond PI")
